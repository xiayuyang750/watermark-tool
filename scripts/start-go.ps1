# start-go.ps1 —— 启动 Go 后端 + 网页前端（浏览器预览模式）
# 用法: powershell -ExecutionPolicy Bypass -File scripts\start-go.ps1

$ErrorActionPreference = "Stop"

$Root       = Split-Path -Parent $PSScriptRoot
$BackendExe = Join-Path $Root "backend-go\wm-backend.exe"
...
Write-Host "=== Watermark Tool 启动器（Go 后端） ===" -ForegroundColor Cyan

# 后端可执行文件检查
if (-not (Test-Path $BackendExe)) {
    Write-Error "未找到 $BackendExe，请先在 backend-go 目录执行 go build"; exit 1
}
...
if (Test-Port 17890) {
    ...
    Write-Host "[backend] 启动 Go 后端..."
    Start-Process -FilePath $BackendExe `
        -WorkingDirectory (Join-Path $Root "backend-go") -WindowStyle Hidden `
        -RedirectStandardOutput (Join-Path $LogsDir "backend-go.out.log") `
        -RedirectStandardError  (Join-Path $LogsDir "backend-go.err.log") | Out-Null
}
