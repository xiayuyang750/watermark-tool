# start-python.ps1 —— 启动 Python(FastAPI) 后端 + 网页前端（浏览器预览模式）
# 用法: powershell -ExecutionPolicy Bypass -File scripts\start-python.ps1
...
Write-Host "=== Watermark Tool 启动器（Python 后端） ===" -ForegroundColor Cyan

# 依赖检查
if (-not (Test-Cmd python)) { Write-Error "未找到 Python，请先安装 Python 3.11+"; exit 1 }
& python -c "import fastapi, uvicorn, httpx, playwright" 2>$null
if ($LASTEXITCODE -ne 0) {
    Write-Host "[setup] 安装后端依赖..." -ForegroundColor DarkYellow
...
if (Test-Port 17890) {
    ...
    Write-Host "[backend] 启动 Python 后端..."
    Start-Process -FilePath "python" -ArgumentList "run.py" `
        -WorkingDirectory $Backend -WindowStyle Hidden `
        -RedirectStandardOutput (Join-Path $LogsDir "backend-py.out.log") `
        -RedirectStandardError  (Join-Path $LogsDir "backend-py.err.log") | Out-Null
}
